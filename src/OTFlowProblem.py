...
def OTFlowProblem(x, Phi, tspan , nt, stepper="rk4", alph =[1.0,1.0,1.0]):
    ...
    costL  = torch.mean(z[:,-2]) # l2 transport cost
    costC  = torch.mean(C(z)) # D_{KL} (flow(x, T) || normal)
    costR  = torch.mean(z[:,-1]) # HJB Regularizer Potential model
    
    wandb.log({"raw_costC": costC}) # this should be close to 0 for a good flow
...
def C(z):
    ...
    d = z.shape[1]-3
    l = z[:,d] # log-det (N_EX, 1)
    
    wandb.log({"norm": torch.mean(torch.sum(torch.pow(z[:, 0:d], 2)/2, 1)).item(), # torch.mean over the samples
               "logdet": torch.mean(l).item()}) # log-det of change in density

    return -( torch.sum(  -0.5 * math.log(2*math.pi) - torch.pow(z[:,0:d],2) / 2  , 1 , keepdims=True ) + l.unsqueeze(1) ) # (N_EX, 1)
