...
parser.add_argument(
    '--data', choices=['power', 'gas', 'hepmass', 'miniboone', 'bsds300', 'prosumer'], type=str, default='miniboone'
)
...
def load_data(name):
    ...
    if name == 'bsds300':
        ...
        return datasets.PROSUMER(args.data_split_path)
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for d1 in range(0, d-1, 2):  # plot 2-D slices of the multivariate distribution
            ...
            im1, _, _, map1 = axs[0, 0].hist2d(testData[:, d1], testData[:, d2], range=[[testData[:, d1].min(), testData[:, d1].max()], [testData[:, d2].min(), testData[:, d2].max()]], bins=nBins)
            axs[0, 0].set_title(r'$x \sim \rho_0(x)$') # source distribution (feature d1, feature d2)

            im2, _, _, map2 = axs[0, 1].hist2d(modelFx[:, d1], modelFx[:, d2], range=bounds, bins=nBins)
            axs[0, 1].set_title(r'$f(x)$') # flow on source distribution just looking at (d1, d2)

            im3, _, _, map3 = axs[1, 0].hist2d(normSamples[:, d1], normSamples[:, d2], range=bounds, bins=nBins)
            axs[1, 0].set_title(r'$y \sim \rho_1(y)$') # ? this is just a torch.randn

            im4, _, _, map4 = axs[1, 1].hist2d(modelGen[:, d1],modelGen[:, d2], range=[[modelGen[:, d1].min(), modelGen[:, d1].max()], [modelGen[:, d2].min(), modelGen[:, d2].max()]], bins=nBins)
            axs[1, 1].set_title(r'$f^{-1}(y)$') # inverse flow on torch.randn
